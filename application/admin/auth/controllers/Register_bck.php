            ...
            //     debug($data) ;die;
                ...
                // } else {
                    ...
                    if($res['status'] == 'success') {
                        $c = [];
                        foreach($nama_keluarga as $k1 => $f) {
                            if($f != '') {
                                $c[$k1]		= [
                                    'id_pelamar' => $res['id'],
                                    'nama'		=> $f,
                                    'tempat_tgl_lahir'	=> $ttl_keluarga[$k1],
                                    'pendidikan'		=> $pddk_keluarga[$k1],
                                    'jabatan'	        => $jabatan_keluarga[$k1],
                                    'institusi'			=> $institusi_keluarga[$k1],
                                ];
                            }
                        }
    
                        delete_data('tbl_keluarga_pelamar','id_pelamar',$res['id']);
    
                        if(count($c)) {
                            $save 	= insert_batch('tbl_keluarga_pelamar',$c);
                        }

                        $c2 = [];
                        foreach($nama_saudara as $k2 => $f2) {
                            if($f2 != '') {
                                $c2[$k2]		= [
                                    'id_pelamar' => $res['id'],
                                    'nama'		=> $f2,
                                    'tempat_tgl_lahir'	=> $ttl_saudara[$k2],
                                    'pendidikan'		=> $pddk_saudara[$k2],
                                    'jabatan'	        => $jabatan_saudara[$k2],
                                    'institusi'			=> $institusi_saudara[$k2],
                                ];
                             }
                        }
    
                        delete_data('tbl_saudara_pelamar','id_pelamar',$res['id']);
    
                        if(count($c2)) {
                            $save 	= insert_batch('tbl_saudara_pelamar',$c2);
                        }

                        $c3 = [];
                        foreach($nama_anak as $k3 => $f3) {
                            if($f3 != '') {
                                $c3[$k3]		= [
                                    'id_pelamar' => $res['id'],
                                    'nama'		=> $f3,
                                    'tempat_tgl_lahir'	=> $ttl_anak[$k3],
                                    'pendidikan'		=> $pddk_anak[$k3],
                                    'jabatan'	        => $jabatan_anak[$k3],
                                    'institusi'			=> $institusi_anak[$k3],
                                ];
                            }
                        }
    
                        delete_data('tbl_anak_pelamar','id_pelamar',$res['id']);
    
                        if(count($c3)) {
                            $save 	= insert_batch('tbl_anak_pelamar',$c3);
                        }
