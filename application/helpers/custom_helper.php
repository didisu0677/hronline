...
function template_pdf($data=[],$key='',$periode='') {
    ...
    $template 		= get_data('tbl_template_surat',[
        'where'     => [
            'key'   => $key,
            'periode <='    => $periode
        ],
...
{
	list($tanggal,$bulan,$tahun) = explode('-',$tanggal_lahir);
	$lahir = mktime(0, 0, 0, (int)$bulan, (int)$tanggal, $tahun); 
	$cari_umur = ($lahir < 0) ? ( $t + ($lahir * -1) ) : time() - $lahir;
	$tahun = 60 * 60 * 24 * 365; // 60 detik * 60 menit * 24 jam * 365 hari
	$tahun_lahir = $cari_umur / $tahun;
	return floor($tahun_lahir); // ubah nilai menjadi pembulatan
}